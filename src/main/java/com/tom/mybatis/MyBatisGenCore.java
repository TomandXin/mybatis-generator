...
class MyBatisGenCore {
     ...
     * @throws Exception 抛出错误
     */
    private static List<Map<String, String>> getColInfoList(String table, Properties properties) throws Exception {
        Connection cn = getConnection(properties);
        try {
     ...
     * @throws Exception 抛出错误
     */
    private static List<Map<String, String>> getSqlmapParamList(List<Map<String, String>> paramList, String pks)
        throws Exception {
        List<Map<String, String>> list = new ArrayList<>();
     ...
     * @throws Exception 抛出错误
     */
    private static void gen(String table, Properties properties) throws Exception {

        List<Map<String, String>> colInfoList = getColInfoList(table, properties);
        ...
        String doTemplate = readFileToString(MyBatisGenConst.DO_TEMPLATE);
        String queryTemplate = readFileToString(MyBatisGenConst.QUERY_TEMPLATE);
        String sqlmapTemplate = readFileToString(MyBatisGenConst.SQLMAP_TEMPLATE);
        String mapperTemplate = readFileToString(MyBatisGenConst.MAPPER_TEMPLATE);
        String managerTemplate = readFileToString(MyBatisGenConst.MANAGER_TEMPLATE);
        String managerImplTemplate = readFileToString(MyBatisGenConst.MANAGER_IMPL_TEMPLATE);
        String sqlmapExtTemplate = readFileToString(MyBatisGenConst.SQLMAP_EXT_TEMPLATE);
        String mapperExtTemplate = readFileToString(MyBatisGenConst.MAPPER_EXT_TEMPLATE);
        String pageResultTemplate = readFileToString(MyBatisGenConst.PAGE_RESULT_TEMPLATE);
        String baseQueryTemplate = readFileToString(MyBatisGenConst.BASE_QUERY_TEMPLATE);
        String baseCriteriaTemplate = readFileToString(MyBatisGenConst.BASE_CRITERIA_TEMPLATE);
        ...
        param.put(MyBatisGenConst.VP_MAPPER_PROPERTY_NAME,
            className.substring(0, 1).toLowerCase() + className.substring(1) + MyBatisGenConst.MAPPER_EXT_SUFFIX);
        ...
        String pageResult = merge(pageResultTemplate, param);
        String baseQueryResult = merge(baseQueryTemplate, param);
        String baseCriteriaResult = merge(baseCriteriaTemplate, param);

        String doOutFilePath = MyBatisGenConst.MAPPER_DO_DIR + "/" + className + MyBatisGenConst.DO_SUFFIX + ".java";
        String queryOutFilePath = MyBatisGenConst.MAPPER_QUERY_DIR + "/" + className + MyBatisGenConst.QUERY_PREFIX
            + ".java";
        String sqlmapOutFilePath = MyBatisGenConst.MAPPER_XML_DIR + "/" + className + MyBatisGenConst.MAPPER_SUFFIX
            + ".xml";
        String mapperOutFilePath = MyBatisGenConst.MAPPER_JAVA_DIR + "/" + className + MyBatisGenConst.MAPPER_SUFFIX
            + ".java";
        String managerOutFilePath = MyBatisGenConst.MANAGER_JAVA_DIR + "/" + className + MyBatisGenConst.MANAGER_SUFFIX
            + ".java";
        String managerImplOutFilePath = MyBatisGenConst.MANAGER_IMPL_JAVA_DIR + "/" + className
            + MyBatisGenConst.MANAGER_IMPL_SUFFIX + ".java";
        String sqlmapExtOutFilePath = MyBatisGenConst.MAPPER_EXT_XML_DIR + "/" + className
            + MyBatisGenConst.MAPPER_EXT_SUFFIX + ".xml";
        String mapperExtOutFilePath = MyBatisGenConst.MAPPER_EXT_JAVA_DIR + "/" + className
            + MyBatisGenConst.MAPPER_EXT_SUFFIX + ".java";
        String pageResultFilePath = MyBatisGenConst.BASE_DIR+ "/PageResult.java";
        String baseQueryFilePath = MyBatisGenConst.BASE_DIR + "/BaseQuery.java";
        String baseCriteriaFilePath = MyBatisGenConst.BASE_DIR + "/BaseCriteria.java";
        ...
        File pageResultOutFile = new File(pageResultFilePath);
        if (!pageResultOutFile.exists()) {
            createAndWriteFile(pageResult,pageResultFilePath,pageResultOutFile);
        }

        File baseQueryOutFile = new File(baseQueryFilePath);
        if (!baseQueryOutFile.exists()) {
            createAndWriteFile(baseQueryResult,baseQueryFilePath,baseQueryOutFile);
        }

        File baseCriteriaOutFile = new File(baseCriteriaFilePath);
        if (!baseCriteriaOutFile.exists()) {
            createAndWriteFile(baseCriteriaResult, baseCriteriaFilePath, baseCriteriaOutFile);
        }
    }

    private static void createAndWriteFile(String managerResult, String managerOutFilePath, File managerOutFile)
        throws Exception {
        boolean success;
    ...
    private static String readFileToString(String filePath) throws Exception {
        InputStream inputStream = MyBatisGenCore.class.getResourceAsStream(filePath);
     ...
     * @throws Exception 抛出异常
     */
    void batchGen(List<String> tables, Properties properties) throws Exception {
        System.out.println("table num " + tables.size());
        Connection cn = getConnection(properties);
        try {
            for (String table : tables) {
                MyBatisGenCore.gen(table.trim(), properties);
